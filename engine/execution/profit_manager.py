...
_LOT_UNITS    = 65
_TRAIL_PCT    = 0.70     # fraction of peak profit retained once cost recovered
                         # (raised 0.62->0.70: 0.62 was cutting winners well
                         # below the reward side of the 3.5R target, while
                         # losers still rode the full initial SL — see
                         # 2026-09 review: avg win Rs141 vs avg loss Rs210)
_ARM_MULT     = 2.5      # MFE must clear this multiple of cost before the
                         # ladder arms (raised 1.5->2.5: 1.5x let ordinary
                         # noise trip an early, tight lock)
...
def ladder_locked_rs(max_pnl: float, qty: int = _LOT_UNITS):
    ...
    if   max_pnl >= 2000.0:
        ...
        locked = max(locked, 0.75 * max_pnl)
        stage  = "S5_LOCK75%"
    elif max_pnl >= 800.0:
        locked = max(locked, 0.72 * max_pnl)
        stage  = "S4_TRAIL72%"
    else:
        stage  = "S1_COSTLOCK" if locked <= cost + 1e-6 else "S2_TRAIL70%"
