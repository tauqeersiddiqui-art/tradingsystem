...
class Phase55Telemetry:
    ...
    def _format_report(payload: dict[str, Any]) -> str:
        ...
        return (
            ...
            f"- Blocked by CE Threshold: {stats['blocked_by_ce_threshold']}\n"
            f"- Blocked by PE Threshold: {stats['blocked_by_pe_threshold']}\n"
            f"- Blocked by Mixed Regime: {stats['blocked_by_mixed_regime']}\n"
            f"- Correct Blocks: {stats['correct_blocks']}\n"
            f"- Incorrect Blocks: {stats['false_positive_blocks']}\n"
            f"- Correct Block Rate: {stats['correct_block_rate']:.2f}\n"
            f"- False Block Rate: {stats['false_block_rate']:.2f}\n"
