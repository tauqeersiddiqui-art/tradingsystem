...
# CE-specific floor — MUST mirror engine/live_engine.py's _CE_ML_FLOOR.
# Without this, the backtest lets CE trades in at 0.65 while live actually
# requires 0.78, so backtest results don't represent live behavior.
_CE_ML_FLOOR = 0.78
...
class BacktestSignalEngine:
    ...
    def step(self, window: pd.DataFrame, ts: datetime) -> Optional[dict]:
        ...
        ce_thr   = max(threshold - 0.03 if (ce_break and orb_ok) else threshold, _CE_ML_FLOOR)
