...
#
# DASHBOARD BUG FIXES (dashboard path only — trading logic untouched):
#
#   FIX-1 : Removed recursion from send_or_edit_engine_dashboard() and
#            send_or_edit_market_dashboard().  Recursion caused TypeError
#            when _send() returned None (rate-limit/timeout), because the
#            call to result["message_id"] exploded.  That exception escaped
#            the stack, was caught by the outer engine_loop try/except, and
#            the stale message ID was never cleared from .telegram_state.json.
#            Every subsequent cycle reloaded the stale ID, re-entered the same
#            failure path, and the dashboard silently died forever.
#
#   FIX-2 : _send_dashboard_fresh() now guards result before indexing.
#            If Telegram returns None (network error), the failure is logged
#            and the in-memory ID stays None so the next cycle retries.
#
#   FIX-3 : _edit() and _edit_with_markup() now detect Telegram error strings
#            "message to edit not found" and "message can't be edited" and
#            return a sentinel ("GONE") so callers can distinguish "edit
#            technically failed but message exists" from "message is gone".
#
#   FIX-4 : send_or_edit_engine_dashboard() and send_or_edit_market_dashboard()
#            act on the GONE sentinel: clear the stale ID from memory AND from
#            .telegram_state.json immediately, then create a fresh message.
#            If the fresh send also fails, the ID stays None and will be
#            retried next cycle.  No more infinite stale-ID loop.
#
#   FIX-5 : Diagnostic logging added for every dashboard step:
#              - attempt (which slot, current message ID)
#              - Telegram API response body on failure
#              - success/failure result per cycle
#            Logged at DEBUG level to avoid chat spam; visible in file logs.
#
...
# Sentinel returned by _edit / _edit_with_markup when Telegram says the
# message is permanently gone (deleted or from a dead session).
_EDIT_GONE = "GONE"
...
def _load_state():
    ...
    try:
        ...
        logger.debug(
            f"[DASH] State loaded — engine_id={_engine_msg_id} market_id={_market_msg_id}"
        )
    except Exception:
        pass
...
def _save_state():
    try:
        ...
        logger.warning(f"[DASH] State save failed: {e}")
...
def _send(chat_id, text, parse_mode="HTML", reply_markup=None,
    ...
    try:
        ...
        d = r.json()
        if not d.get("ok"):
            logger.warning(f"[TG] Send error: {d}")
            return None
        return d.get("result")
    except Exception as e:
...
# FIX-3: returns True (success), False (transient failure), or _EDIT_GONE
def _edit(message_id, text, parse_mode="HTML"):
    """
    Edit a message.
    Returns:
        True       — success (or no-op because text unchanged)
        False      — transient failure (network / unknown API error)
        _EDIT_GONE — message permanently gone; caller should create a fresh one
    """
    ...
    try:
        ...
        logger.debug(
            f"[DASH] editMessageText id={message_id} "
            f"ok={d.get('ok')} desc={d.get('description','')}"
        )
        ...
        # FIX-3: detect permanently gone messages
        if any(phrase in desc for phrase in (
            ...
            logger.warning(
                f"[DASH] Message {message_id} is GONE (Telegram: {d.get('description')}). "
                "Will create fresh dashboard message."
            )
            return _EDIT_GONE

        logger.warning(f"[DASH] Edit failed (transient): {d}")
        return False

    except Exception as e:
        logger.warning(f"[DASH] Edit exception: {e}")
        return False


# FIX-3 variant for markup edits
def _edit_with_markup(message_id, text, reply_markup=None):
    ...
    try:
        ...
        logger.debug(
            f"[DASH] editMessageText(markup) id={message_id} "
            f"ok={d.get('ok')} desc={d.get('description','')}"
        )
        ...
        if any(phrase in desc for phrase in (
            ...
            logger.warning(
                f"[DASH] Market message {message_id} is GONE "
                f"(Telegram: {d.get('description')}). "
                "Will create fresh market dashboard message."
            )
            return _EDIT_GONE

        logger.warning(f"[DASH] Market edit failed (transient): {d}")
        return False

    except Exception as e:
        logger.warning(f"[DASH] Market edit exception: {e}")
        return False
...
# ─────────────────────────────────────────────────────────────────────
# INTERNAL: create a fresh dashboard message and persist its ID
# FIX-2: guards result before indexing; no TypeError on None
# ─────────────────────────────────────────────────────────────────────

def _send_engine_fresh(text: str) -> bool:
    """Send a new engine dashboard message; persist ID. Returns True on success."""
    global _engine_msg_id
    logger.info("[DASH] Creating fresh engine dashboard message")
    result = _send(BOT_CHAT_ID, text)
    if result is None:
        logger.warning("[DASH] Fresh engine send returned None — will retry next cycle")
        return False
    new_id = result.get("message_id")
    if not new_id:
        logger.warning(f"[DASH] Fresh engine send: unexpected result shape {result}")
        return False
    _engine_msg_id = new_id
    _last_edited.pop(new_id, None)   # force real edit on next cycle
    _save_state()
    logger.info(f"[DASH] Engine dashboard created: message_id={new_id}")
    return True


def _send_market_fresh(text: str, reply_markup=None) -> bool:
    """Send a new market dashboard message; persist ID. Returns True on success."""
    global _market_msg_id
    logger.info("[DASH] Creating fresh market dashboard message")
    result = _send(BOT_CHAT_ID, text, reply_markup=reply_markup)
    if result is None:
        logger.warning("[DASH] Fresh market send returned None — will retry next cycle")
        return False
    new_id = result.get("message_id")
    if not new_id:
        logger.warning(f"[DASH] Fresh market send: unexpected result shape {result}")
        return False
    _market_msg_id = new_id
    _last_edited.pop(new_id, None)
    _save_state()
    logger.info(f"[DASH] Market dashboard created: message_id={new_id}")
    return True


# ─────────────────────────────────────────────────────────────────────
# DUAL DASHBOARD  — two persistent edit-in-place messages
# FIX-1: no recursion; FIX-4: clear stale IDs on GONE signal
# ─────────────────────────────────────────────────────────────────────
...
def send_or_edit_engine_dashboard(text: str):
    ...
    logger.debug(f"[DASH] Engine dashboard refresh — current id={_engine_msg_id}")

    if _engine_msg_id is None:
        # No stored ID: send fresh
        _send_engine_fresh(text)
        return

    # Try to edit the existing message
    result = _edit(_engine_msg_id, text)

    if result is True:
        logger.debug(f"[DASH] Engine dashboard edited OK (id={_engine_msg_id})")
        return

    if result is _EDIT_GONE:
        # FIX-4: message gone — clear stale ID immediately, send fresh
        logger.warning(
            f"[DASH] Clearing stale engine_msg_id={_engine_msg_id} from state"
        )
        _last_edited.pop(_engine_msg_id, None)
        _engine_msg_id = None
        _save_state()                   # persist cleared ID immediately
        _send_engine_fresh(text)        # create new message (no recursion)
        return

    # result is False: transient failure (network). Keep existing ID, retry next cycle.
    logger.warning(
        f"[DASH] Engine dashboard edit had transient failure "
        f"(id={_engine_msg_id}) — will retry next cycle"
    )
...
def send_or_edit_market_dashboard(text: str, reply_markup=None):
    ...
    logger.debug(f"[DASH] Market dashboard refresh — current id={_market_msg_id}")

    if _market_msg_id is None:
        _send_market_fresh(text, reply_markup)
        return

    result = _edit_with_markup(_market_msg_id, text, reply_markup)

    if result is True:
        logger.debug(f"[DASH] Market dashboard edited OK (id={_market_msg_id})")
        return

    if result is _EDIT_GONE:
        logger.warning(
            f"[DASH] Clearing stale market_msg_id={_market_msg_id} from state"
        )
        _last_edited.pop(_market_msg_id, None)
        _market_msg_id = None
        _save_state()
        _send_market_fresh(text, reply_markup)
        return
    ...
    logger.warning(
        f"[DASH] Market dashboard edit had transient failure "
        f"(id={_market_msg_id}) — will retry next cycle"
    )
...
def reset_dashboard():
    ...
    _last_edited.clear()
    _save_state()
    logger.info("[DASH] Dashboard state reset — fresh messages will be created next cycle")
...
def send_trade_entry_with_exit_button(message):
    ...
    if result:
        _trade_msg_id = result.get("message_id")
...
def poll_commands(status_cb=None):
    ...
    try:
        ...
        for update in data.get("result", []):
            ...
            if "callback_query" in update:
                ...
                if action == "manual_exit":
                    ...
                    if _pending_confirm_id and action.endswith(
                        _pending_confirm_id.split("_", 1)[1]
                    ):
        ...
        logger.warning(f"[TG] poll_commands error: {e}")
