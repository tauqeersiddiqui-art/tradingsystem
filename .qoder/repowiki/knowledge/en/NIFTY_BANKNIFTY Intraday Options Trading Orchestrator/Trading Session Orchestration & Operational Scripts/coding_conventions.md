...
- External configuration is loaded lazily from `.env` via `dotenv.load_dotenv` inside functions instead of at module import time.
...
- Windows-safe process checks use `psutil.pid_exists()` plus cmdline verification instead of `os.kill(pid, 0)` to avoid false positives on Windows.